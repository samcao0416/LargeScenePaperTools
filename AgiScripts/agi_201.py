...
parser.add_argument("--intrin", type = str, default = None, help = "path of precalibrated intrinsic file")
...
if xml_path is not None:
    ...
    chunk.importReference(path=export_reference_path, format=Metashape.ReferenceFormatCSV, items=Metashape.ReferenceItemsCameras, columns = "n||||||xyzabc", delimiter=',')

    # 6. 修改reference中xyz的误差为0.03m
    chunk.camera_location_accuracy = (0.1, 0.1, 0.1)

    # 7. 修改相机模型为鱼眼模型，并导入预标定文件
    if intrin_xml is not None:
        camera = chunk.cameras[0]
        camera.sensor.type = Metashape.Sensor.Type.Fisheye
        calib = Metashape.Calibration()
        calib.load(path = intrin_xml,format = Metashape.CalibrationFormatXML)
