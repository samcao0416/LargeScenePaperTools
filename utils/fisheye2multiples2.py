...
class Fisheye2Multiples():
    ...
    def generate_points(self, views: list = ["F", "L","R", "U", "D"]):

        self.views = views
        self.points = self._generate_points_with_raycasting(views=views)

    
    def fisheye_to_multiples(self, cam_pose, output_folder:str = None, write_mode: bool = True):
        ...
        for i, new_cam_pose in enumerate(self.new_cam_pose_list):
            ...
            pointsreshape  = self.points[i].reshape(-1, 3)
