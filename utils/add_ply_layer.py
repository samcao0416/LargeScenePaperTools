...
x_min, y_min, _ = min_bound
x_max, y_max, _ = max_bound

# 3. 在x_min到x_max和y_min到y_max这个距离内，每隔0.1增加一个白色的点
x_range = np.arange(x_min, x_max, 0.1)
y_range = np.arange(y_min, y_max, 0.1)
X, Y = np.meshgrid(x_range, y_range)
Z = np.zeros_like(X)  # 假设在平面上，Z坐标为0

# 创建点云
new_points = np.vstack((X.flatten(), Y.flatten(), Z.flatten())).T
